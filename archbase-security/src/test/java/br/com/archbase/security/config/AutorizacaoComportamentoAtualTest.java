 ...
 *
 * <p>O comportamento de {@code ArchbaseSecurityService.hasPermission} já está coberto por
 * {@code ArchbaseSecurityServiceTest}; aqui ficam as lacunas — os quatro managers de anotação e as
 * bordas que ninguém testava.
 */
...
class AutorizacaoComportamentoAtualTest {
    ...
    class Perfil {
        ...
        void administradorIsentoPorPadrao() throws Exception {
            ...
            // seria barrado por um @RequireProfile que ele não satisfaz. O core precisa continuar
            // honrando allowSystemAdmin como isenção declarada DESTA restrição.
            ArchbaseSecurityService service = mock(ArchbaseSecurityService.class);
            ProfileAuthorizationManager manager = new ProfileAuthorizationManager(service);
        ...
        @Test
        @DisplayName("com resource declarado, a restrição consulta o catálogo — portão 3 chamando o 5")
        void comResourceDelegaAoCatalogo() throws Exception {
            // Restrição e concessão já estão misturadas hoje. O core separa as duas em requisitos
            // distintos, mas o resultado observável precisa permanecer igual.
            ArchbaseSecurityService service = mock(ArchbaseSecurityService.class);
            when(service.hasPermission(any(), anyString(), anyString(), any(), any(), any())).thenReturn(false);
            ProfileAuthorizationManager manager = new ProfileAuthorizationManager(service);
            ...
            verify(service).hasPermission(any(), org.mockito.ArgumentMatchers.eq("VIEW"),
                    org.mockito.ArgumentMatchers.eq("PRODUTO"), any(), any(), any());
        }
        ...
        void semPerfilNaoConsultaCatalogo() throws Exception {
            ...
            assertThat(permitido(manager, user, Alvo.class, "exigeAdminEPermissao")).isFalse();
            verify(service, never()).hasPermission(any(), anyString(), anyString(), any(), any(), any());
        }
        ...
        @Test
        @DisplayName("principal que não é UserEntity nega — a exceção é engolida")
        void principalEstranhoNega() throws Exception {
            // ClassCastException no cast do principal cai no catch e vira negação. Numa aplicação
            // com UserDetailsService próprio isso barra todo mundo, com stack trace no log.
            // O core corrige na fase A; aqui fica o registro do que acontece hoje.
            ArchbaseSecurityService service = mock(ArchbaseSecurityService.class);
            ProfileAuthorizationManager manager = new ProfileAuthorizationManager(service);
        ...
        @Test
        @DisplayName("isAdministrator nulo nega por NullPointerException engolida")
        void administradorNuloNega() throws Exception {
            // user.getIsAdministrator() é desempacotado sem checagem. Usuário gravado sem a flag
            // — possível, a coluna aceita nulo — vira NPE, catch, negação silenciosa.
            ArchbaseSecurityService service = mock(ArchbaseSecurityService.class);
            ProfileAuthorizationManager manager = new ProfileAuthorizationManager(service);
    ...
    class Persona {
        ...
        void administradorIsentoPorPadrao() throws Exception {
            ArchbaseSecurityService service = mock(ArchbaseSecurityService.class);
            PersonaAuthorizationManager manager = new PersonaAuthorizationManager(service);
        ...
        void mapeamentoFixoDeOutroDominio() throws Exception {
            // validateBasicPersonaMapping traz PLATFORM_ADMIN/STORE_ADMIN/CUSTOMER/DRIVER
            // embutidos — vocabulário de outro domínio, dentro do framework. O perfil ADMIN casa
            // com a persona PLATFORM_ADMIN por essa tabela, não por configuração do cliente.
            ArchbaseSecurityService service = mock(ArchbaseSecurityService.class);
            PersonaAuthorizationManager manager = new PersonaAuthorizationManager(service);
    ...
    class Papel {
        ...
        void varioResolversSaoUnidos() throws Exception {
        ...
        private RoleAuthorizationManager roleManager(List<ArchbaseRoleResolver> resolvers, String politica) {
            RoleAuthorizationManager manager = new RoleAuthorizationManager();
            ReflectionTestUtils.setField(manager, "roleResolvers", resolvers);
            ReflectionTestUtils.setField(manager, "noResolverPolicy", politica);
            return manager;
        }
    }
    ...
    class Capacidade {
        ...
        void delegaAoCatalogo() throws Exception {
            ArchbaseSecurityService service = mock(ArchbaseSecurityService.class);
            when(service.hasPermission(any(), anyString(), anyString(), any(), any(), any())).thenReturn(true);

            CustomAuthorizationManager manager = new CustomAuthorizationManager(service);
        ...
        void tenantDaAnotacaoVence() throws Exception {
            ...
            CustomAuthorizationManager manager = new CustomAuthorizationManager(service);
            permitido(manager, usuario("user-1", false), Alvo.class, "editarProdutoDeTenantFixo");

            verify(service).hasPermission(any(), anyString(), anyString(),
                    org.mockito.ArgumentMatchers.eq("tenant-fixo"), any(), any());
        }
        ...
        void erroNaAvaliacaoNega() throws Exception {
            ArchbaseSecurityService service = mock(ArchbaseSecurityService.class);
            when(service.hasPermission(any(), anyString(), anyString(), any(), any(), any()))
    ...
    private static boolean permitido(org.springframework.security.authorization.AuthorizationManager<MethodInvocation> manager,
