...
import static br.com.archbase.validation.fluentvalidator.predicate.ComparablePredicate.greaterThan;
import static br.com.archbase.validation.fluentvalidator.predicate.ComparablePredicate.lessThan;
import static br.com.archbase.validation.fluentvalidator.predicate.LogicalPredicate.is;
import static br.com.archbase.validation.fluentvalidator.predicate.LogicalPredicate.not;
import static br.com.archbase.validation.fluentvalidator.predicate.ObjectPredicate.equalObject;
...
public final class StringPredicate {
  ...
  public static Predicate<String> isUUID() {
    return PredicateBuilder.<String>from(not(stringEmptyOrNull())).and(isUUID -> {
      return UUID_PATTERN_1.matcher(isUUID).matches() ||
              UUID_PATTERN_2.matcher(isUUID).matches() ||
              UUID_PATTERN_3.matcher(isUUID).matches() ||
              UUID_PATTERN_4.matcher(isUUID).matches() ||
              UUID_PATTERN_5.matcher(isUUID).matches() ||
              UUID_PATTERN_6.matcher(isUUID).matches();
    });
  }
